<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('manage.rosters.update', $roster) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $roster->name)" required autofocus />
                                ...
                                <select id="department_id" name="department_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    ...
                                    @foreach ($departments as $department)
                                        <option value="{{ $department->id }}" 
                                            {{ old('department_id', $roster->department_id) == $department->id ? 'selected' : '' }}>
                                            {{ $department->name }}
                                        </option>
                                    @endforeach
                                </select>
                                <x-input-error :messages="$errors->get('department_id')" class="mt-2" />
                                <p class="text-sm text-gray-500 mt-1">{{ __('You must select a department.') }}</p>
                            </div>
                            
                            <div>
                                <x-input-label for="staff_type" :value="__('Staff Type')" />
                                <select id="staff_type" name="staff_type" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" {{ !$roster->department_id ? 'disabled' : '' }}>
                                    <option value="">{{ __('Select Staff Type') }}</option>
                                    @foreach ($staffTypes as $value => $label)
                                        <option value="{{ $value }}" {{ old('staff_type', $roster->staff_type) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                <x-input-error :messages="$errors->get('staff_type')" class="mt-2" />
                                
                                @if ($roster->entries()->count() > 0)
                                    <p class="text-sm text-yellow-600 mt-1">{{ __('Warning: Changing staff type may affect existing roster entries.') }}</p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <div id="roster_type_display" class="mt-2 p-3 bg-gray-100 rounded-md border border-gray-300 h-10 flex items-center">
                                    <span id="roster_type_value" class="font-medium">
                                        @if($roster->roster_type)
                                            {{ $roster->roster_type === 'oncall' ? 'On Call' : 'Shift' }}
                                        @else
                                            {{ __('Select department and staff type first') }}
                                        @endif
                                    </span>
                                </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="start_date" class="block mt-1 w-full" type="date" name="start_date" :value="old('start_date', $roster->start_date->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="end_date" class="block mt-1 w-full" type="date" name="end_date" :value="old('end_date', $roster->end_date->format('Y-m-d'))" required />
                                <x-input-error :messages="$errors->get('end_date')" class="mt-2" />
                            </div>
                            
                            <div class="md:col-span-2">
                                <x-input-label for="description" :value="__('Description (Optional)')" />
                                <textarea id="description" name="description" rows="3" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description', $roster->description) }}</textarea>
                                <x-input-error :messages="$errors->get('description')" class="mt-2" />
                            </div>
                            
                            <div class="md:col-span-2">
                                <label for="is_published" class="inline-flex items-center mt-4">
                                    <input id="is_published" type="checkbox" name="is_published" value="1" {{ old('is_published', $roster->is_published) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-600">{{ __('Publish roster') }}</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('manage.rosters.show', $roster) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150 ml-3">
                                {{ __('Cancel') }}
                            </a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Store department roster configurations
            ...
            // If department and staff type are pre-selected (from existing roster), update immediately
            ...
            // Initialize with current roster type color
            if (rosterTypeValue.textContent.trim() === 'On Call') {
                rosterTypeValue.classList.add('text-orange-600');
            } else if (rosterTypeValue.textContent.trim() === 'Shift') {
                rosterTypeValue.classList.add('text-blue-600');
            }
            ...
            function updateRosterTypeDisplay() {
                ...
                if (staffType && departmentId) {
                    ...
                    if (departmentConfig && departmentConfig.staff_type_rosters) {
                        ...
                        const staffTypeRoster = departmentConfig.staff_type_rosters.find(
                            config => config.staff_type === staffType
                        );
            ...
            // Initialize roster type display on page load
            updateRosterTypeDisplay();
        });
    </script>
</x-app-layout> 