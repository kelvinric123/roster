<nav x-data="{ open: false, rostersOpen: false, settingsOpen: false, staffOpen: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <x-nav-link :href="route('departments.index')" :active="request()->routeIs('departments.*')">
                        {{ __('Department & Roster Management') }}
                    </x-nav-link>
                    @endif
                    
                    <!-- Staff Nav Link with Dropdown -->
                    <div class="hidden sm:flex sm:items-center" @click.away="staffOpen = false">
                        <div class="relative" x-data="{ open: false }">
                            <button @click="staffOpen = !staffOpen" class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out {{ request()->routeIs('staff.*') || request()->routeIs('team-leaders.*') ? 'text-gray-900 border-indigo-500' : '' }}">
                                {{ __('Staff') }}
                            ...
                            <div x-show="staffOpen" 
                                ...
                                <div class="rounded-md ring-1 ring-black ring-opacity-5 py-1 bg-white">
                                    @if($isAdmin || $isDepartmentLeader)
                                    <a href="{{ route('staff.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 {{ request()->routeIs('staff.index') ? 'bg-gray-100' : '' }}">
                                        {{ __('Staff Management') }}
                                    </a>
                                    @endif
                                    @if($isAdmin)
                                    <a href="{{ route('team-leaders.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 {{ request()->routeIs('team-leaders.*') ? 'bg-gray-100' : '' }}">
                                        {{ __('Team Leaders') }}
                                    </a>
                                    @endif
                                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <!-- Responsive Staff Dropdown -->
            <div @click.away="staffOpen = false" class="relative">
                <button @click="staffOpen = !staffOpen" class="flex w-full items-center pl-3 pr-4 py-2 border-l-4 {{ request()->routeIs('staff.*') || request()->routeIs('team-leaders.*') ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300' }} text-base font-medium focus:outline-none transition duration-150 ease-in-out">
                    <div>{{ __('Staff') }}</div>
                    <div class="ml-1">
                        <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
